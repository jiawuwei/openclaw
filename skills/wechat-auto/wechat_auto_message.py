#!/usr/bin/env python3
"""
微信消息自动化脚本 - Python版本 v2.0
使用Python自动化控制微信桌面客户端进行消息发送
基于模板匹配技术，不使用硬编码坐标
参考wechat-call-python结构
"""
...
class Config:
    def __init__(self):
        ...
        self.user = None
        self.message = None
        self.confidence_threshold = 0.85  # 稍低的阈值，因为输入框更难匹配
    ...
    def validate(self):
        ...
        # 如果提供了完整格式的消息，尝试解析
        if "给" in self.user and "发" in self.user:
            parts = self.user.split("给")[1]
            if "发" in parts:
                self.user = parts.split("发")[0].strip()
                self.message = parts.split("发")[1].strip()
                return True, ""
        
        if not self.message:
            return False, "请使用 --message 参数指定消息内容"
...
class Utils:
    ...
    @staticmethod
    def extract_user_and_message(full_text):
        """从消息中提取用户名和消息内容"""
        if not full_text:
        ...
        if match:
            ...
            logger.log(f"从消息提取: 用户='{username}', 消息='{message}'")
            return username, message
        else:
            # 如果没有匹配到完整格式，返回原始文本作为用户名
            logger.log(f"未匹配到完整格式，使用原始文本: '{full_text}'")
    ...
    def cleanup_desktop_screenshots():
        ...
        try:
            ...
            if system == "Windows":
                ...
                logger.log(f"警告: 未知操作系统 {system}，跳过清理截图", "warn")
                return 0
            ...
            if deleted_count > 0:
                logger.log(f"清理完成: 删除了 {deleted_count} 个桌面截图文件", "success")
            else:
...
class TemplateMatcher:
    ...
    def smart_scan(self, template_name, scan_area=None):
        ...
        # 模拟模式下返回模拟匹配结果
        if config.simulate:
            logger.debug_log(f"[模拟] 跳过真实匹配，返回模拟位置")
            # 返回一个模拟的匹配结果
            return {
                "x": 100,
                "y": 100,
...
class WeChatSender:
    ...
    def run(self):
        """主流程：发送微信消息"""
        logger.log("开始微信消息自动化流程 - Python版本", "info")
        logger.log("说明: 使用模板匹配技术，无硬编码坐标", "info")
        
        try:
            # 1. 提取用户名和消息内容
            if "给" in config.user and "发" in config.user:
                username, message_text = self.utils.extract_user_and_message(config.user)
                logger.log(f"目标用户: {username}", "info")
                logger.log(f"消息内容: {message_text}", "info")
            else:
            ...
            # 8. 复制消息内容到剪贴板
            logger.log("步骤5: 复制消息内容到剪贴板", "info")
            self.utils.copy_to_clipboard(message_text)
            self.utils.delay(500)
            
            # 9. 查找并点击输入框
            logger.log("步骤6: 查找并点击输入框", "info")
            input_match = self.matcher.smart_scan("input")
            
            self.ops.click_match_result(input_match, "点击输入框")
            self.utils.delay(300)
            
            # 10. 粘贴消息内容
            logger.log("步骤7: 粘贴消息内容", "info")
            self.ops.paste_from_clipboard("从剪贴板粘贴消息内容")
            self.utils.delay(300)
            
            # 11. 发送消息
            logger.log("步骤8: 发送消息", "info")
            self.ops.press_key("enter", "按回车发送消息")
            self.utils.delay(500)
            
            # 12. 完成报告
            logger.log("\n微信消息发送流程完成!", "success")
            logger.log(f"已成功发送消息给: {username}", "success")
            logger.log(f"消息内容: {message_text}", "success")
            logger.log("技术特点:", "success")
            logger.log("- 纯Python实现，无Node.js依赖", "success")
            logger.log("- 基于模板匹配技术（搜索框+输入框）", "success")
            ...
            logger.log("3. 可以降低置信度阈值: --confidence 0.7", "error")
            logger.log("4. 检查依赖是否已安装", "error")
            logger.log("5. 确保微信已登录并且有该用户的对话", "error")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="微信消息自动化脚本 - Python版本")
    parser.add_argument("--user", required=True, help="目标用户名或完整消息，如'用微信给可乐煮鸡发今天不吃饭'")
    parser.add_argument("--message", help="消息内容（如果不在--user中）")
    ...
    parser.add_argument("--confidence", type=float, default=0.85, help="模板匹配置信度阈值 (0.0-1.0, 默认0.85)")
